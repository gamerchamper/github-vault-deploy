...
function renderServerHistory(servers,activeId){serverCatalog=servers||[];const sel=$('serverHistory');if(!sel)return;const opts=['<option value="">Saved servers…</option>'].concat(serverCatalog.map(s=>'<option value="'+esc(s.id)+'"'+(s.id===activeId?' selected':'')+'>'+esc(s.label)+' · '+esc(s.apiKeyPreview||s.cookiePreview||'session')+'</option>'));sel.innerHTML=opts.join('');$('removeServer').disabled=!sel.value;updateButtons()}
...
async function useSelectedServer(){const id=$('serverHistory').value;if(!id)return;setState('Connecting…','busy');try{const r=await api('/api/servers/select',{method:'POST',body:JSON.stringify({id})});await loadConfig();setState(r.authenticated?'Connected':'Auth failed',r.authenticated?'good':'bad')}catch(e){setState(e.message,'bad')}finally{updateButtons()}}
async function removeSelectedServer(){const id=$('serverHistory').value;if(!id)return;if(!confirm('Remove this server from history?'))return;const data=await api('/api/servers/'+encodeURIComponent(id),{method:'DELETE'});renderServerHistory(data.servers||[],'');await loadConfig()}
  ...
  return http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/api/config') {
        ...
        if (body.cookie !== undefined) config.cookie = String(body.cookie || '');
