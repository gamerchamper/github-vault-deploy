...
const $=id=>document.getElementById(id);let activeTask=null;let pollTimer=null;let selectedFiles=[];let uploadQueue=null;const state={busy:false,configured:false};
...
function updateButtons(){const hasFile=getUploadPaths().length>0;$('planBtn').disabled=state.busy||!hasFile||!state.configured;$('uploadBtn').disabled=state.busy||!hasFile||!state.configured;$('saveConfig').disabled=state.busy;$('browseBtn').disabled=state.busy}
function getUploadBody(){return{parentPath:$('parentPath').value,mode:$('mode').value,concurrency:$('concurrency').value,chunkSize:$('chunkSize').value,convertHls:$('convertHls').checked}}
...
function renderPlan(plan){renderFeedback(plan.feedback);if(plan.plans&&plan.plans.length>1){$('planOut').innerHTML='<div class="plan-summary"><div><span>Files</span><strong>'+plan.totalFiles+'</strong></div><div><span>Total size</span><strong>'+bytes(plan.totalSize)+'</strong></div><div><span>Total chunks</span><strong>'+plan.totalChunks+'</strong></div></div><div class="list" style="margin-top:10px;max-height:200px">'+plan.plans.map(p=>'<div class="item"><strong>'+esc(p.fileName)+'</strong><div class="muted">'+bytes(p.size)+' · '+p.totalChunks+' chunks</div></div>').join('')+'</div>';return}const p=plan.plans?plan.plans[0]:plan;$('planOut').innerHTML='<div class="plan-summary"><div><span>Chunks</span><strong>'+p.totalChunks+'</strong></div><div><span>Chunk size</span><strong>'+bytes(p.chunkSize)+'</strong></div><div><span>Estimated time</span><strong>'+esc(p.estimatedTime||'--')+'</strong></div><div><span>Repos</span><strong>'+p.repoCount+'</strong></div></div>'}
...
function validateUpload(){if(!getUploadPaths().length)throw new Error('Choose a file first');if(!state.configured)throw new Error('Configure server credentials first')}
function updateJobUi(j){const queueLabel=uploadQueue&&uploadQueue.paths.length>1?' · '+(uploadQueue.index+1)+'/'+uploadQueue.paths.length:'';$('activeStatus').textContent=j.status+queueLabel;$('progressFill').style.width=((j.progress&&j.progress.percent)||0)+'%';$('chunks').textContent=((j.progress&&j.progress.chunksDone)||0)+'/'+((j.progress&&j.progress.totalChunks)||0);$('speed').textContent=speed(j.progress&&j.progress.speed);$('logs').textContent=(j.logs||[]).map(l=>new Date(l.at).toLocaleTimeString()+' '+l.message).join('\\n')||j.error||j.status}
function waitForJob(taskId){return new Promise(resolve=>{const tick=async()=>{try{const j=await api('/api/jobs/'+encodeURIComponent(taskId));updateJobUi(j);if(['done','error','paused'].includes(j.status))resolve(j);else setTimeout(tick,500)}catch(e){setState('Waiting for local job status','busy');setTimeout(tick,1200)}};tick()})}
async function startUpload(resumeTaskId){validateUpload();const btn=$('uploadBtn');const paths=resumeTaskId?[$('filePath').value.trim()]:getUploadPaths();if(!paths.length)throw new Error('Choose a file first');setBusy(btn,true,'Starting');uploadQueue=paths.length>1&&!resumeTaskId?{paths,index:0}:null;let completed=0;try{for(let i=0;i<paths.length;i++){if(uploadQueue)uploadQueue.index=i;const label=paths.length>1?'Uploading '+(i+1)+'/'+paths.length+': '+basename(paths[i]):'Uploading';setState(label,'busy');const body={...getUploadBody(),filePath:paths[i]};if(resumeTaskId&&i===0)body.resumeTaskId=resumeTaskId;const r=await api('/api/upload',{method:'POST',body:JSON.stringify(body)});activeTask=r.taskId;const j=await waitForJob(activeTask);if(j.status==='error')throw new Error(j.error||'Upload failed');if(j.status==='paused'){setState('paused');break}completed++;resumeTaskId=null}if(completed===paths.length){setState(paths.length>1?'All uploads complete':'done','good');if(paths.length>1){selectedFiles=[];renderSelectedFiles()}}}catch(e){setState(e.message,'bad');$('logs').textContent=e.stack||e.message}finally{uploadQueue=null;setBusy(btn,false);refreshSessions();refreshRemote()}}
async function pollJob(){if(!activeTask)return;clearTimeout(pollTimer);try{const j=await api('/api/jobs/'+encodeURIComponent(activeTask));updateJobUi(j);if(['done','error','paused'].includes(j.status)){setState(j.status,j.status==='done'?'good':j.status==='error'?'bad':'');refreshSessions();refreshRemote()}else pollTimer=setTimeout(pollJob,500)}catch(e){setState('Waiting for local job status','busy');pollTimer=setTimeout(pollJob,1200)}}
async function chooseFiles(){if(!window.vaultDesktop){setState('Native file picker is available in desktop mode only','bad');return}const pick=window.vaultDesktop.selectFiles||window.vaultDesktop.selectFile;const result=await pick();const filePaths=result&&(result.filePaths||[]).length?result.filePaths:(result&&result.filePath?[result.filePath]:[]);if(result&&!result.canceled&&filePaths.length){selectedFiles=filePaths;renderSelectedFiles();setState(filePaths.length+' file(s) selected','good')}}
...
async function planUpload(){validateUpload();const btn=$('planBtn');const paths=getUploadPaths();setBusy(btn,true,'Planning');setState('Planning upload','busy');try{const body={chunkSize:$('chunkSize').value};if(paths.length>1)body.filePaths=paths;else body.filePath=paths[0];const r=await api('/api/plan',{method:'POST',body:JSON.stringify(body)});renderPlan(r);setState('Plan ready','good')}catch(e){setState(e.message,'bad');$('planOut').textContent=e.message}finally{setBusy(btn,false)}}
