...
function isPermanentUploadError(msg) {
    const lower = msg.toLowerCase();
    return (/exceed the .* limit/.test(lower)
    ...
    try {
        ...
        if (msg.includes('already exists') && entry.fileId) {
            ...
            try {
                ...
                logger_1.logger.error('upload-queue', `Replace retry failed for ${entry.localRelPath}: ${retryMsg}`);
            }
        }
...
function shouldConvertHls(settings, fileName, mimeType) {
    if (settings.convertHlsEnabled === false)
        return false;
